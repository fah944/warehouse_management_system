      ...
      builder: (context, state) {
        if (state is PendingRequestLoading) {
          ...
          if (state.pendingRequests.isEmpty) {
            return const Center(
                child: Text('No pending courses available.',
    ...
    return BlocBuilder<PendingBeneficiaryCubit, PendingBeneficiaryState>(
      builder: (context, state) {
        if (state is PendingBeneficiaryLoading) {
          ...
          if (state.pendingRequests.isEmpty) {
            return const Center(
                child: Text('No pending beneficiaries available.',
                    style: TextStyle(fontSize: 16)));
          }
          return ListView.builder(
            padding: const EdgeInsets.all(8),
      ...
      builder: (context, state) {
        if (state is PendingTrainerLoading) {
          ...
          if (state.pendingTrainers.isEmpty) {
            return const Center(
                child: Text('No pending trainers available.',
                    style: TextStyle(fontSize: 16)));
          }
          return ListView.builder(
            padding: const EdgeInsets.all(8),
              ...
              return Card(
                ...
                child: ListTile(
                  ...
                  subtitle: Column(
                    ...
                    children: [
                      const SizedBox(height: 4),
                      Text('Email: ${pendingTrainer?.email ?? 'N/A'}',
                          style: TextStyle(color: Colors.grey[700])),
                      Text('Status: ${pendingTrainer.status}',
                          style: TextStyle(color: Colors.grey[700])),
                    ],
                  ),
                  trailing: const Icon(Icons.arrow_forward_ios,
